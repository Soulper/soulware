...
# IP to User mapping for Guardian
IP_USER_MAP_FILE = os.path.join(settings.DATA_DIR, 'ip_user_map.json')
...
async def receive_data(
    ...
    if log_type in SKIP_LOG_TYPES:
        ...
        if log_type in ('screenshot', 'webcam', 'screen', 'image'):
            ...
            save_dir = os.path.join(settings.DATA_DIR, build_key, folder_name)
...
async def guardian_screenshot(
    ...
    screenshot_dir = os.path.join(settings.DATA_DIR, user.build_key, "screenshots")
    os.makedirs(screenshot_dir, exist_ok=True)
